...
export class SalaryHistoryComponent implements OnInit {
  ...
  historyId: any;
  empId: any;
  pdfGeneration: boolean = true;
  printFuntion: boolean = true;
  ...
  ngOnInit(): void {
    ...
    this.salary_month = this.months[m - 1];
    ...
    this.aRoute.params.subscribe(param => {
      ...
      combineLatest(
        ...
        if (typeof this.data.bank === 'string') { this.data.bank = JSON.parse(window.atob(this.data.bank)) }
  ...
  addToSalary(form) {
    ...
    let month = moment().subtract(1, 'month').format('MMM-YYYY').split('-').join('_');

    this.salaryService.addMonthlyAllowance(this.id, this.empId, month, formValue)
      .then(() => {
  ...
  confirmation(confirm: boolean) {
    if (confirm === true) {
      this.salaryService.deleteHistory(this.id, this.historyId)
        .then(() => {
          this.deleted = true;
  ...
  printHistory() {
    if (Object.keys(this.history).length != 0 && Object.keys(this.data).length != 0) { MakePrint.history(document.getElementById('details')); }
    else {
      this.printFuntion = false;
      setTimeout(() => {
        this.printFuntion = true;
      }, 2000)
    }
  }
  ...
  makePdf() {
    ...
    if (Object.keys(this.history).length != 0 && Object.keys(this.data).length != 0) {
      let pdf = MakePdf.salaryHistory(this.history, this.data);
      pdf.open()
    } else {
      this.pdfGeneration = false;
      setTimeout(() => {
        this.pdfGeneration = true;
      }, 2000)
    }
