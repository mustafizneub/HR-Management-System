...
@Component({
  selector: 'app-viewadminleave',
  templateUrl: './viewadminleave.component.html',
  styleUrls: ['./viewadminleave.component.css']
})
export class ViewadminleaveComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.db.collection('leave-request', ref => ref.where('status', '==', 1).where('role', '==', 'admin')).valueChanges().subscribe(object => {
      this.data = object;
    });
  ...
  accepted() {
    ...
    this.db.collection('leave-request', ref => ref.where('leavestatus', '==', "approve").where('status', '==', 1).where('role', '==', 'admin')).valueChanges().subscribe(object => {
      this.data = object;
  ...
  pending() {
    ...
    this.db.collection('leave-request', ref => ref.where('leavestatus', '==', "pending").where('status', '==', 1).where('role', '==', 'admin')).valueChanges().subscribe(object => {
      this.data = object;
  ...
  reject() {
    ...
    this.db.collection('leave-request', ref => ref.where('leavestatus', '==', "reject").where('status', '==', 1).where('role', '==', 'admin')).valueChanges().subscribe(object => {
      this.data = object;
  ...
  edit(id) {
    ...
    this.router.navigate(['/adminleaveedit', id]);
