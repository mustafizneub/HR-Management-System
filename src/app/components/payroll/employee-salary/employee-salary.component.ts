...
export class EmployeeSalaryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    combineLatest(
      ...
      this.salaryService.getEmployeeSalary(this.id).get().subscribe(e => {
        ...
        this.bonus_type = employeeSalary.bonus_type;
        this.prev_bonus_type = this.bonus_type;
        this.bonus_amount = employeeSalary.bonus_amount
  ...
  update(form) {
    ...
    if (this.type != 'Monthly Salary') {
      ...
      else if (this.amount < this.prev_amount) {
    ...
    if (this.prev_bonus_amount < this.bonus_amount) {
    ...
    // this.salaryService.getEmployeeSalary(this.id).collection('salary-history').doc(this.historyId).set(formValue, { merge: true }).then(() => {
    //   this.salaryService.getEmployeeSalary(this.id).collection('salary-history').doc(this.historyId).collection('activity-log').add(changed)
    // })

    // console.log(formVal);
    if (formVal.staff_salary != undefined) {
      // UPDATE FUNCTION
      this.salaryService.getEmployeeSalary(this.id).set(formVal, { merge: true }).then(() => {
